...
private:
	...
	std::ifstream mapfile_cellandbin;
	...
	std::set < int > list_of_bins_ecal_NEW;
	std::map< int, std::set<int> > cells_by_binglobal_ecal_NEW;
	std::map < int, int > binglobal_by_cell_ecal_NEW;


public:

	Ecalbin( const std::string& ecal_mapfile = "/w/halla-scshelf2102/sbs/adr/g4sbs/g4sbs/database/ecal_gep_blockmap.txt", const std::string& ecal_cellandbin_mapfile = "ecal_cell_table_gep_feb10.txt" )
	: mapfile_ecal(ecal_mapfile), mapfile_cellandbin(ecal_cellandbin_mapfile)
	{
		TString currentline;
		...
		// Now read the ecal cell and bin map file to assign bins to each ECal cell: Feb10 file from Kip.
		if ( !mapfile_cellandbin.is_open() ) std::cerr << "Error: Could not open file!\n";

    	std::string line;

    	while ( std::getline(mapfile_cellandbin, line) ) 
    	{
	        ...
	        int cell, row, col, slot, ch, bin, txfiber, txch, rxfiber, rxch, globalBin;
	        std::string rocid, rxrocid;

	        // Read values from the processed line
	        if (ss >> cell >> row >> col >> rocid >> slot >> ch >> bin >> txfiber >> txch >> rxrocid >> rxfiber >> rxch >> globalBin) 
	        {
	        	list_of_bins_ecal_NEW.insert(globalBin);
	            binglobal_by_cell_ecal_NEW[cell] = globalBin;
	            cells_by_binglobal_ecal_NEW[globalBin].insert(cell);
	        } 
	        else 
	        {
	...
	void ECalClustering( int n_ecalhits, std::vector<int>* ecalhit_row, std::vector<int>* ecalhit_col, std::vector<int>* ecalhit_cell, std::vector<double>* ecalhit_sumedep, std::vector<double>* ecalhit_xcell, std::vector<double>* ecalhit_ycell )
	{
		...
		while ( !ECALblocks_unused.empty() )
		{
			...
			if( nodes_ecal_by_cluster_center.find((*maxblk).cell) != nodes_ecal_by_cluster_center.end() )
			{
				...
				thiscluster.bin = binglobal_by_cell_ecal_NEW[(*maxblk).cell]; //binglobal_by_cell_ecal[(*maxblk).cell];
	...
	int numOfECalBinsNew()
	{
		return list_of_bins_ecal_NEW.size();
	}

	std::map< int, std::set<int> >& returnCellsByBinGlobalNewMap()
	{
	...
	TList* returnECalDiagramPerBinTListPtr( const int binNum )
	{
		...
		if ( list_of_bins_ecal_NEW.find(binNum) == list_of_bins_ecal_NEW.end() ) // See whether the input ECal bin number exists.
	    {
	    ...
	    for (const auto &cell : xcell_ecal) 
	    {
	        ...
	        if ( binglobal_by_cell_ecal_NEW[cellnum] == binNum ) // See if the current cell belongs to the input ECal bin number.
	        {
	        	...
	        	const double text_Xpos = xcell_ecal[cellnum] - xspace/2.5;
	...
	std::set < int >& returnListOfBinsECalNewMap()
	{
		return list_of_bins_ecal_NEW;
	}
